...
export function calculateAttritionFeatures(candidate = {}, simulation = {}) {
  ...
  const market = Number(compensation.market) || 0;
  const current = Number(compensation.current) || 0;
  const salaryGap = market > 0 ? (market - current) / market : 0.45;
  ...
  return applySimulation({
    ...
    NoticeRisk: clamp(Number(engagement.absencesLastQuarter ?? 1) / 10),
