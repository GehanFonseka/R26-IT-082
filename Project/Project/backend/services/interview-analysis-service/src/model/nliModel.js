...
export const modelStatus = () => ({ loaded: state.loaded, model: env.nliModelId, error: state.error?.message || null });

export const loadModel = async () => {
  if (state.loaded) return;
  if (!state.promise) {
    state.promise = Promise.all([
      AutoTokenizer.from_pretrained(env.nliModelId),
      AutoModelForSequenceClassification.from_pretrained(env.nliModelId),
...
const classify = async (premise, hypothesis) => {
  if (!state.loaded) throw Object.assign(new Error("Interview NLI model is not loaded"), { statusCode: 503 });
