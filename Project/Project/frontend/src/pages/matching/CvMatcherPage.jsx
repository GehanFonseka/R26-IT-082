...
function CvMatcherPage() {
  ...
  useEffect(() => { getMyProfile().then((response) => { updateUser({ profilePhoto: response.data?.profilePhoto || "" }); const saved = response.data?.cv?.candidate; if (saved) setCandidate((current) => ({ ...current, ...toMatcherCandidate(saved) })); }).catch(() => {}); }, []);
  const retentionCandidate = useMemo(() => ({ ...candidate, displayName: candidate.displayName || "this candidate" }), [candidate]);
  useEffect(() => {
    let mounted = true;
  ...
  useEffect(() => { getAttritionAssessment(retentionCandidate, defaultSimulation).then(setBaselineRetentionRisk).catch(() => setBaselineRetentionRisk(null)); }, [retentionCandidate]);
    ...
    <div className="cv-app-shell">
      ...
      <div className="cv-app-shell__main">
        ...
        {activeView === "analysis" ? <CvAnalysisPage job={job} candidate={candidate} result={result} selectedFile={selectedFile} retentionCandidate={retentionCandidate} retentionSimulation={retentionSimulation} retentionRisk={retentionRisk} baselineRetentionRisk={baselineRetentionRisk} retentionLoading={retentionLoading} onRetentionSimulationChange={setRetentionSimulation} onResetRetentionSimulation={() => setRetentionSimulation(defaultSimulation)} onGoToMatcher={() => navigate("/matching")} /> : <main className="cv-matcher-page" id="cv-matcher">
          ...
          <footer className="cv-matcher-page__footer"><span>Assistive ranking only - always review the original CV and job requirements.</span><a href="https://huggingface.co/Gehan77/cv-match-browser" target="_blank" rel="noreferrer">Model card ↗</a></footer>
        </main>}
      </div>
    </div>
  );
