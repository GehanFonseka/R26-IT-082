...
import { explainMatch as requestExplanation } from "../services/explanationClient.js";
...
export const explain = async (req, res) => {
  const { rawText, candidate, job, matchResult } = req.body ?? {};
  if (typeof rawText !== "string" || !candidate || !job || !matchResult) throw Object.assign(new Error("rawText, candidate, job and matchResult are required"), { statusCode: 400 });
  const explanation = await requestExplanation({ rawText, candidate, job, matchResult, requestId: req.requestId });
